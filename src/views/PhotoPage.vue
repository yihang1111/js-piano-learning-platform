<template>
    ...
    <div class="top-bar">
        <HeaderBar v-if="false"></HeaderBar>
        <div class="logo">
            <img src="@/assets/images/piano.png" class="logo-img"/>
            <p class="logo-name">钢琴知识学习系统</p>
        </div>
        <div class="menu">
            <DemoButton @click="goAutoPlay" button_size="max" :button_status="(state.id == -1 ? 'cprimary' : 'csecondary')" class="menu-AutoPlay">自由弹奏</DemoButton>
            <DemoButton @click="goForum" button_size="max" :button_status="(state.id == 0 ? 'cprimary' : 'csecondary')" class="menu-Forum">学习论坛</DemoButton>
            <DemoButton @click="goPhoto" button_size="max" :button_status="(state.id == 1 ? 'cprimary' : 'csecondary')" class="menu-Music">曲谱中心</DemoButton>
        ...
        <div class="user">
            ...
            <!--游客状态==> 去登录 -->
            <div class="user-head" v-if="this.user.id == '::1'" @click="goPerson"></div>
            <!--登录状态 ==> 退出登录 变游客  -->
            <div class="user-head" v-if="this.user.id != '::1'" @click="exit"></div>
        </div>
    </div>
    <div class="wall-message" >
        ...
        <div class="label" :class="{ 'fixed': isNavbarFixed || wFlag || PFlag}">
            ...
            <p class="label-list" :class="{labelSelected:label_num == index}" v-for="(item,index) in label[1]" :key="index" @click="SwitchListNode(index)">{{item}}</p>
        </div>
        ...
        <div class="photo">
            ...
            <MusicPhoto :photo="item" class="music-photo " @clickbtn="deletePhoto" @toPhotoDetail="selectPhoto(index)"  v-for="(item,index) in photos" :key="index" ></MusicPhoto>
        ...
        <!-- 新建图片按键 -->
        <div class="add" v-if="this.user.id != '::1'" @click="newPhoto" v-show="cardSelected == -1 && addFlag">
        ...
        <PhotoView @changePFlag="changePFlag" v-show=" PFlag" :id1="id1" :label="label">
            <PhotoDetail :id1="id1" :np="np" v-if="!np" :photo="photos[photoSelected1]"></PhotoDetail>
            <PhotoDetail2 :id1="id1" :np="np" v-if="np" :photo="photos[photoSelected1]"></PhotoDetail2>
            <PostPhoto :np="np" :photo="photos[photoSelected1]" @closePW="closePW" >
                <PhotoComment :id1="id1" :np="np" :photo1="photos[photoSelected1]" @clickbt="insertPhoto"></PhotoComment>
            </PostPhoto>
        </PhotoView>
