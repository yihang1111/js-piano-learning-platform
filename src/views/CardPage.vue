<template>
    ...
    <div class="wall-message" >
        ...
        <div class="label" :class="{ 'fixed': isNavbarFixed || wFlag || PFlag}">
            ...
            <p class="label-list" :class="{labelSelected:label_num == index}" v-for="(item,index) in label[0]" :key="index" @click="SwitchListNode(index)">{{item}}</p>
        </div>
        ...
        <PostCard :title="title" @clickbtn="deleteCard" @changeW="changePostW" :wFlag="wFlag" :id="id1" :label="label" :rr="rr">
            <NewCard :id="id1" @newCardClose="closePostW" v-show="cardSelected == -1" @clickbtn="insert" ></NewCard>
            <CardDetail :id="id1" v-show="cardSelected != -1 && wFlag " :cards="cards[cardSelected]" ></CardDetail>
        </PostCard>
