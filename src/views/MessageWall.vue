<template>
  <div class="wall-message">
    ...
    <div class="labelfixed" v-show="wFlag">
      <!-- 动态类绑定时，属性名中不允许使用连字符 -。因此，在你的代码中，label-selected 应该改为 labelSelected 或者使用字符串形式的类名。 -->
      <p class="label-list" :class="{ labelSelected: labelnum == -1 }" @click="SwitchListNode(-1)">
        全部
      </p>
      <p
        class="label-list"
        :class="{ labelSelected: labelnum == index }"
        v-for="(item, index) in label[id]"
        :key="index"
        @click="SwitchListNode(index)"
      >
    ...
    >
      <NewCard
        ...
        v-show="cardSelected == -1 && id == 0"
        @clickbtn="insert"
      />
      <CardDetail :id="id" v-show="cardSelected != -1 && wFlag" :cards="cards[cardSelected]" />
    </PostCard>

    <!-- @changePFlag="changePFlag" -->
    <PhotoView @changePFlag="changePFlag" v-show="id == 1 && PFlag" :id="id" :label="label">
      <PhotoDetail :id="id" :np="np" v-if="!np" :photo="photos[photoSelected1]" />
      <PhotoDetail2 :id="id" :np="np" v-if="np" :photo="photos[photoSelected1]" />
      <PostPhoto :np="np" :photo="photos[photoSelected1]" @closePW="closePW">
        <PhotoComment :id="id" :np="np" :photo1="photos[photoSelected1]" @clickbt="insertPhoto" />
      </PostPhoto>
    </PhotoView>
  </div>
</template>
